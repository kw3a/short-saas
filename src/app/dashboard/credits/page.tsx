  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold inline-flex items-center gap-2"><CircleDollarSign size={18} className="text-yellow-400" /> Credit History</h1>
          <p className="text-sm text-zinc-400">Track spent credits and other adjustments.</p>
        </div>
        <button
      ...
      <div className="flex items-center gap-3">
        <button onClick={() => setTab("spends")} className={`px-3 py-1.5 rounded border ${tab === "spends" ? "bg-zinc-800 text-white border-zinc-700" : "bg-transparent text-zinc-300 border-zinc-800"}`}>Spent Credits</button>
        <button onClick={() => setTab("adjustments")} className={`px-3 py-1.5 rounded border ${tab === "adjustments" ? "bg-zinc-800 text-white border-zinc-700" : "bg-transparent text-zinc-300 border-zinc-800"}`}>Other Adjustments</button>
      </div>

      {tab === "spends" && (
        <div className="mb-2">
          <h2 className="text-base font-medium mb-2">Spent Credits</h2>
          {spends.length === 0 ? (
            <div className="text-sm text-zinc-400">No spend events yet.</div>
          ) : (
                ...
                <tbody>
                  {spends.map((s) => (
                    <tr key={s.id} className="border-t border-zinc-800">
                      ...
                      <td className="px-3 py-2 text-red-400">-{(s.creditCost ?? 0).toLocaleString()} credits</td>
      ...
      {tab === "adjustments" && (
        <div>
          <h2 className="text-base font-medium mb-2">Other Adjustments</h2>
          {adjustments.length === 0 ? (
            <div className="text-sm text-zinc-400">No adjustments yet.</div>
          ) : (
            <div className="overflow-x-auto border border-zinc-800 rounded-md">
              <table className="min-w-full text-sm">
                <thead className="bg-zinc-900 text-zinc-300">
                  ...
                  {adjustments.map((a) => (
                    <tr key={a.id} className="border-t border-zinc-800">
                      ...
                      <td className={`px-3 py-2 ${a.amount >= 0 ? "text-green-400" : "text-red-400"}`}>
                        {a.amount >= 0 ? "+" : ""}{(a.amount ?? 0).toLocaleString()} credits
                      </td>
